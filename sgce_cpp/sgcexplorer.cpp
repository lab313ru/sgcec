...
void byteWrite(unsigned long addr, byte value, int CARTState) {
    ...
    for(i = firstDataPin; i <= lastDataPin; i++) {
...
void doINFO() {
    Serial.println("Welcome to SGC Explorer - v2.3.13");
    Serial.println("Based on Bruno Freitas's SGCE v2.3.12");
        Serial.println("06/2013 - Dr. MefistO[Lab 313]");
...
void loop() {
    ...
    for (;;) {
        ...
        if (strncasecmp(token, "READ", MAX_SERIAL_STRING_SIZE) == 0) {
            ROMReadWord();
        } else if (strncasecmp(token, "WRITE", MAX_SERIAL_STRING_SIZE) == 0) {
            ROMWriteWord();
